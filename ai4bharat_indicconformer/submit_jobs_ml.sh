#!/bin/bash
# Local script to submit HF Jobs for AI4Bharat IndicConformer-600M-Multi evaluation.
# IndicConformer covers the 22 scheduled Indian languages (no English); of the
# leaderboard's multilingual sets only Hindi overlaps, via VoiceArena/Monsoon_hi_test.
# NOTE: the model repo is gated — HF_TOKEN must belong to an account that has
# accepted https://huggingface.co/ai4bharat/indic-conformer-600m-multilingual terms.
# This script is NOT pushed to the HF Space — it runs on your local machine.
# Usage: HF_TOKEN=hf_... bash submit_jobs_ml.sh

# ── Configuration ────────────────────────────────────────────────────────────
SPACE="${SPACE:-bezzam/open-asr-leaderboard-ai4bharat-indicconformer}"
...
FLAVOR="${FLAVOR:-h200}"
ORG_NAME="${ORG_NAME:-}"
# Decoding head of this hybrid CTC+RNNT model: "rnnt" (usually more accurate)
# or "ctc" (single forward pass, much faster). Worth running both once.
DECODING="${DECODING:-rnnt}"
...
# batch_size only groups rows for datasets.map — the shipped model decodes one
# utterance at a time either way (see run_eval_ml.py's module docstring).
MODEL_CONFIGS=(
    "ai4bharat/indic-conformer-600m-multilingual      64"
)
...
# Indic languages IndicConformer supports).
DATASET_CONFIGS=(
    "monsoon hi"
)
...
for model_cfg in "${MODEL_CONFIGS[@]}"; do
    ...
    echo "████████████████████████████████████████████████████████████████████████████████"
    echo "  Evaluating: ${MODEL_ID} (decoding=${DECODING})"
    echo "████████████████████████████████████████████████████████████████████████████████"
