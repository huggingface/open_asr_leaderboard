...
`edits_voxpopuli.jsonl` (one row per disagreement) in `--out-dir`.
"""
...
from ref_error_utils import DEFAULTS, ref_error_agreement, find_ref_errors  # noqa: E402
...
def _make_normalizer():
    ...
    Reference and hypothesis are normalized identically, so that a span only
    counts as a disagreement when it survives the same text normalization the
    leaderboard's WER is computed under.
    """
...
def collect_references(manifests: dict[str, str], dataset: str) -> tuple[list[str], dict[str, str]]:
    ...
    Returns the row order of the first key-bearing manifest and a key-to-text
    map. Every manifest for a dataset carries the same references; disagreement
    means the manifests were produced against different dataset revisions and is
    ...
    for model, path in manifests.items():
        ...
        if keys_in_order is None:
            keys_in_order = keys
        for row in rows:
            previous = text_by_key.setdefault(row["audio_filepath"], row["text"])
            if previous != row["text"]:
                raise ValueError(
                    f"manifests for {dataset} disagree on the reference for "
                    f"{row['audio_filepath']} ({model}); "
                    "they were produced against different dataset revisions"
                )
    if keys_in_order is None:
...
def collect_hypotheses(
    ...
    """
    anchor_text = [None] * len(keys_in_order)
    ...
    for model, path in manifests.items():
        ...
        if not any(key.startswith(OPAQUE_KEY_PREFIX) for key in keys):
            ...
            if anchor_text[0] is None and keys == keys_in_order:
...
def main() -> None:
    ...
    parser.add_argument("--official-dataset", default=OFFICIAL_DATASET)
    parser.add_argument("--corrected-dataset", default=CORRECTED_DATASET)
    ...
    for key in keys:
        ...
        clip_edits = find_ref_errors(
            ...
            min_consensus_cer=DEFAULTS["min_consensus_cer"],
        )
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["model", "rate", "lo", "hi", "n_ref", "n_eligible"])
        for model in sorted(rates):
            row = rates[model]
            writer.writerow(
                [model, f"{row['rate']:.4f}", f"{row['lo']:.4f}", f"{row['hi']:.4f}", row["n_ref"], row["n_eligible"]]
            )
