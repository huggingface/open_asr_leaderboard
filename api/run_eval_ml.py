...
def transcribe_dataset(
        ...
        with checkpoint_path.open("r", encoding="utf-8") as checkpoint_file:
            for line in checkpoint_file:
                ...
                if index in completed:
                    raise ValueError(
                        f"Duplicate sample index {index} in checkpoint {checkpoint_path}"
                    )
                completed[index] = (
                    str(item["reference"]),
                    str(item["prediction"]),
    ...
    sample_count = len(ds)
    invalid_indices = sorted(index for index in completed if index >= sample_count)
    if invalid_indices:
        raise ValueError(
            f"Checkpoint contains out-of-range sample indices: {invalid_indices[:10]}"
        )
